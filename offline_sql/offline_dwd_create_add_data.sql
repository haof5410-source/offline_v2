...
insert overwrite table dwd_trade_cart_add_inc partition (ds = '20251008')
select
    id,
    user_id,
    sku_id,
...
set hive.exec.dynamic.partition.mode=nonstrict;
insert overwrite table dwd_trade_order_detail_inc partition (ds = '20251008')
select
    od.id,
    order_id,
    user_id,
...
set hive.exec.dynamic.partition.mode=nonstrict;
insert overwrite table dwd_trade_pay_detail_suc_inc partition (ds = '20251008')
select
    od.id,
    od.order_id,
    user_id,
...
insert overwrite table dwd_trade_cart_full partition(ds='20251008')
select
    id,
    user_id,
    sku_id,
...
set hive.exec.dynamic.partition.mode=nonstrict;
insert overwrite table dwd_trade_trade_flow_acc partition(ds = '20251008')
select
    oi.id,
    user_id,
    province_id,
...
set hive.exec.dynamic.partition.mode=nonstrict;
insert overwrite table dwd_tool_coupon_used_inc partition(ds = '20251008')
select
    id,
    coupon_id,
    user_id,
...
set hive.exec.dynamic.partition.mode=nonstrict;
insert overwrite table dwd_interaction_favor_add_inc partition (ds = '20251008')
select
    id,
    user_id,
    sku_id,
...
insert overwrite table dwd_traffic_page_view_inc partition (ds='20251008')
select
...
select
    ...
    '20251008' as ds  -- 明确分区字段值，与过滤条件一致
from
    (
        -- 用户注册主信息（解析 data 字段）
        select
...
insert overwrite table dwd_user_login_inc partition (ds = '20251008')
select user_id,
       -- 解析后的 ts 用于时间格式化
